...
hint: use bean to access thread information."
  [^Thread t]
  (let [thread-info (bean t)]
    (str (:id thread-info) "-" (:name thread-info) ":" (:alive thread-info) "," (:interrupted thread-info))))

(defn add-elements!
  "given a java.util.Collection l, add the elements e1 e2 e3 and return the
  modified collection. hint: use the doto macro."
  [l e1 e2 e3]
  (doto l (.add e1) (.add e2) (.add e3)))
...
(defn lexicographical-comparator
  ...
  (reify java.util.Comparator
    (compare [this obj1 obj2] (.compareTo (str obj1) (str obj2)))
    (equals [this obj] (.equals this obj))))
